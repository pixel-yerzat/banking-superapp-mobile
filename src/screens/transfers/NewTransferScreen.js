...
const NewTransferScreen = ({ navigation, route }) => {
  const { recipient: initialRecipient, amount: initialAmount } =
    route.params || {};
  ...
  const [transferByPhone, { isLoading: isPhoneLoading }] =
    useTransferByPhoneMutation();
  const [transferByAccount, { isLoading: isAccountLoading }] =
    useTransferByAccountMutation();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      console.error("Transfer Error:", error.data.errors);
      Alert.alert(
        "Ошибка",
        error.data?.message || "Не удалось выполнить перевод"
      );
    }
  };
          ...
          <View style={styles.typeSelector}>
            {transferTypes.map((type) => (
              <TouchableOpacity
                ...
                style={[
                  styles.typeButton,
                  transferType === type.id && styles.typeButtonActive,
                ]}
              ...
              >
                ...
                <Text
                  style={[
                    styles.typeText,
                    transferType === type.id && styles.typeTextActive,
                  ]}
                >
                  {type.label}
                </Text>
                  ...
                  >
                    <View style={styles.accountOptionInfo}>
                      <Text style={styles.accountOptionName}>
                        {account.name ||
                          `Счёт •••• ${account.account_number.slice(-4)}`}
                      </Text>
