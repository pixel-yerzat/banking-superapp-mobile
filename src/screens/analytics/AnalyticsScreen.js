...
import { useGetCategoryStatsQuery, useGetSpendingAnalysisQuery, useGetMonthlySummaryQuery } from '../../api';
...
const AnalyticsScreen = ({ navigation }) => {
  ...
  const { data: categoryData } = useGetCategoryStatsQuery({ period: selectedPeriod });
  const { data: spendingData } = useGetSpendingAnalysisQuery({ period: selectedPeriod });
        ...
        <View style={styles.summaryContainer}>
          <Card style={[styles.summaryCard, { backgroundColor: colors.primary }]}>
            ...
            <View style={styles.summaryChange}>
              ...
              <Text style={styles.summaryChangeText}>+12% к прошлому месяцу</Text>
            </View>
          </Card>
          ...
          <Card style={[styles.summaryCard, { backgroundColor: colors.success }]}>
            ...
            <View style={styles.summaryChange}>
              ...
              <Text style={styles.summaryChangeText}>+5% к прошлому месяцу</Text>
            </View>
          </Card>
        </View>
        ...
        <Card style={styles.balanceCard}>
          <View style={styles.balanceRow}>
            <View>
              ...
              <Text style={[styles.balanceAmount, { color: (summary.balance || 150000) >= 0 ? colors.success : colors.error }]}>
                {(summary.balance || 150000) >= 0 ? '+' : ''}{formatAmount(summary.balance || 150000)} ₸
              </Text>
            </View>
          ...
          <Card style={styles.pieChartCard}>
              ...
              <View style={styles.pieChart}>
                {categories.length > 0 ? (
                  <View style={styles.pieChartInner}>
                    <Text style={styles.pieChartAmount}>{formatAmount(totalSpending)}</Text>
            ...
            <View style={styles.categoriesList}>
              {(categories.length > 0 ? categories : [
                ...
                { category: 'entertainment', amount: 20000, percentage: 13 },
                ...
                <TouchableOpacity key={index} style={styles.categoryItem}>
                  <View style={styles.categoryLeft}>
                    ...
                    <View style={[styles.categoryIcon, { backgroundColor: `${getCategoryColor(index)}20` }]}>
                      <Ionicons name={getCategoryIcon(cat.category)} size={18} color={getCategoryColor(index)} />
                    </View>
                    <Text style={styles.categoryName}>
                      {cat.category === 'food' ? 'Еда' :
                       cat.category === 'transport' ? 'Транспорт' :
                       cat.category === 'shopping' ? 'Покупки' :
                       cat.category === 'entertainment' ? 'Развлечения' :
                       cat.category === 'utilities' ? 'Коммунальные' :
                       cat.category === 'health' ? 'Здоровье' :
                       cat.category === 'education' ? 'Образование' : 'Другое'}
                    </Text>
                  </View>
                  <View style={styles.categoryRight}>
                    <Text style={styles.categoryAmount}>{formatAmount(cat.amount)} ₸</Text>
                    <Text style={styles.categoryPercentage}>{cat.percentage || Math.round((cat.amount / totalSpending) * 100)}%</Text>
                  </View>
                </TouchableOpacity>
              ))}
            </View>
          ...
          <Card style={styles.insightCard}>
            ...
            <View style={styles.insightContent}>
              ...
              <Text style={styles.insightText}>
                В этом месяце вы потратили на 15% больше на еду. Попробуйте готовить дома чаще.
              </Text>
            </View>
          </Card>
          <Card style={styles.insightCard}>
...
const styles = StyleSheet.create({
  ...
  summaryAmount: { ...typography.h3, color: colors.white, marginVertical: spacing.xs },
  ...
  savingsContainer: { flexDirection: 'row', alignItems: 'center', gap: spacing.xs },
  savingsText: { ...typography.body2, color: colors.success },
  chartCard: { marginHorizontal: spacing.lg, marginBottom: spacing.md },
  chartTitle: { ...typography.body1, fontWeight: '600', color: colors.textPrimary, marginBottom: spacing.md },
  ...
  chartBar: { backgroundColor: colors.primary, borderRadius: 4, width: '100%', minHeight: 4 },
  chartLabel: { ...typography.caption, color: colors.textTertiary, marginTop: spacing.xs },
  ...
  categoryAmount: { ...typography.body2, fontWeight: '600', color: colors.textPrimary },
  ...
  insightTitle: { ...typography.body2, fontWeight: '600', color: colors.textPrimary },
  insightText: { ...typography.caption, color: colors.textSecondary, marginTop: 2 },
});
