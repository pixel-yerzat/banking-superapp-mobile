...
const LOAN_PURPOSES = [
  ...
  { id: 'refinance', label: 'Рефинансирование', icon: 'refresh-outline', rate: 16 },
...
const LoanApplicationScreen = ({ navigation, route }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert('Ошибка', error.data?.message || 'Не удалось отправить заявку');
    }
  };
  ...
  const renderStep1 = () => (
    <View>
      ...
      {LOAN_PURPOSES.map((purpose) => (
        <TouchableOpacity key={purpose.id} onPress={() => setLoanPurpose(purpose)}>
          <Card style={[styles.purposeCard, loanPurpose?.id === purpose.id && styles.purposeCardSelected]}>
            <View style={[styles.purposeIcon, loanPurpose?.id === purpose.id && styles.purposeIconSelected]}>
              <Ionicons
                name={purpose.icon}
                size={24}
                color={loanPurpose?.id === purpose.id ? colors.white : colors.primary}
              />
            </View>
    ...
    <View>
      ...
      <Card style={styles.calcCard}>
        <View style={styles.calcRow}>
          ...
          <Text style={styles.calcValue}>{formatAmount(Math.round(monthlyPayment), 'KZT')}</Text>
        </View>
        <View style={styles.calcRow}>
        ...
        <View style={styles.calcRow}>
          ...
          <Text style={styles.calcValueBold}>{formatAmount(Math.round(totalPayment), 'KZT')}</Text>
        </View>
      </Card>
    </View>
  );
  ...
  const renderStep3 = () => (
    <View>
      ...
      {EMPLOYMENT_TYPES.map((type) => (
        <TouchableOpacity key={type.id} onPress={() => setEmploymentType(type.id)}>
          <Card style={[styles.employmentCard, employmentType === type.id && styles.employmentCardSelected]}>
      ...
      {income && parseInt(income, 10) < monthlyPayment * 2 && (
        <Card style={styles.warningCard}>
          ...
          <Text style={styles.warningText}>
            Рекомендуемый доход для такого платежа: {formatAmount(Math.round(monthlyPayment * 2), 'KZT')}
          </Text>
        </Card>
      )}
    </View>
    ...
    <View>
      ...
      <Card style={styles.summaryCard}>
        <View style={styles.summarySection}>
          ...
          <View style={styles.summaryRow}>
            ...
            <Text style={styles.summaryValue}>{formatAmount(amount, 'KZT')}</Text>
          </View>
          <View style={styles.summaryRow}>
        ...
        <View style={styles.summarySection}>
          ...
          <View style={styles.summaryRow}>
            ...
            <Text style={styles.summaryValueHighlight}>{formatAmount(Math.round(monthlyPayment), 'KZT')}</Text>
          </View>
          <View style={styles.summaryRow}>
            <Text style={styles.summaryLabel}>Переплата</Text>
            <Text style={styles.summaryValue}>{formatAmount(Math.round(overpayment), 'KZT')}</Text>
          </View>
          <View style={styles.summaryRow}>
            <Text style={styles.summaryLabel}>Всего к возврату</Text>
            <Text style={styles.summaryValue}>{formatAmount(Math.round(totalPayment), 'KZT')}</Text>
          </View>
        </View>
      </Card>
      ...
      <TouchableOpacity style={styles.termsRow} onPress={() => setAgreedToTerms(!agreedToTerms)}>
        ...
        <Text style={styles.termsText}>
          Я согласен на проверку кредитной истории и обработку персональных данных
        </Text>
      </TouchableOpacity>
      ...
      <Card style={styles.infoCard}>
        ...
        <Text style={styles.infoText}>
          Решение по заявке принимается в течение 15 минут. После одобрения деньги будут 
          зачислены на ваш счёт.
        </Text>
      </Card>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  sectionLabel: { ...typography.body2, color: colors.textSecondary, fontWeight: '500', marginBottom: spacing.sm },
  purposeCard: { flexDirection: 'row', alignItems: 'center', marginBottom: spacing.sm },
  ...
  purposeInfo: { flex: 1 },
  purposeLabel: { ...typography.body1, fontWeight: '600', color: colors.textPrimary },
  ...
  calcRow: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: spacing.sm },
  calcLabel: { ...typography.body2, color: colors.textSecondary },
  calcValue: { ...typography.body2, fontWeight: '500', color: colors.textPrimary },
  calcLabelBold: { ...typography.body1, fontWeight: '600', color: colors.textPrimary },
  calcValueBold: { ...typography.body1, fontWeight: '700', color: colors.primary },
  calcDivider: { height: 1, backgroundColor: colors.gray200, marginVertical: spacing.sm },
  employmentCard: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  ...
  summaryTitle: { ...typography.body1, fontWeight: '600', color: colors.textPrimary, marginBottom: spacing.sm },
  ...
  summaryValueHighlight: { ...typography.body1, fontWeight: '700', color: colors.primary },
  summaryDivider: { height: 1, backgroundColor: colors.gray100, marginVertical: spacing.sm },
  ...
  infoCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: `${colors.info}10`, gap: spacing.sm },
